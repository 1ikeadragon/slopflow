...
- Before committing to a design, generate at least two structurally different approaches (not surface variants) and compare them on the criteria that matter; see `reasoning-discipline` for option generation and criteria-based evaluation.
- After comparing approaches, commit using `reasoning-discipline`'s Decide move: name the dominant criterion and main tradeoff, prefer a robust option when uncertainty cannot be cheaply reduced, and state what would change the recommendation.
