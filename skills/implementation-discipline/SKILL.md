...
Use this skill when making changes. Make the smallest change that correctly fixes the actual problem, not the smallest change that merely silences a symptom, and not an opportunistic broad rewrite. Do not introduce abstractions for one call site or change public behavior silently.
...
6. Calibrate the scope (in-situ vs. structural), then decide whether to reuse, modify, or create.
...
The most common scope failure is tunnel vision: patching the first obvious line when the real fix is structural, and, less often, widening a small fix into a broad rewrite. Right scope is a deliberate choice, not whatever the request first points at.

Before planning a non-trivial change, hold both options in view and choose on purpose:
...
- Right seam? Is there an existing abstraction or extension point this should go through, or is the local fix patching around it and adding drift?
...
Default to the smallest change that fixes the actual problem. When these questions show in-situ is insufficient, widen scope deliberately instead of band-aiding; when the structural urge is only aesthetic, stay in-situ. For a genuinely close call, use `reasoning-discipline` (Generate options → Decide).

Then commit to a scope and state the alternative you rejected:

- In-situ: name the structural issue you are deliberately leaving, and why (out of scope, needs its own change, too risky now). Do not silently ignore it.
- Structural: justify why the smaller change is insufficient, not merely nicer.

Scale to stakes: an obvious one-line fix or a typo does not need this. Reach for it when the change touches shared code, repeats a pattern, fixes a reported bug, or the obvious fix feels like it is treating a symptom.
...
- for non-trivial approach or scope choices, the structurally different alternatives considered (including in-situ vs. structural scope and the alternative rejected) and why this one wins (see `reasoning-discipline`)
