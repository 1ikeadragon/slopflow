...
For non-trivial investigation, apply `reasoning-discipline` first to frame the problem and structure the reasoning. When a symptom has competing explanations, use its Diagnose move (competing hypotheses plus the evidence that distinguishes them) before concluding.
...
Self-critique is not proof. Rereading the same unsupported answer, or asserting it more confidently, is not verification. Check the central, load-bearing claim, not only the easy ones, against executable evidence or an authoritative source. Do not revise a correct answer just because a critique implies it should change; revise when the evidence changes, not when the prompt pressures you to.
