...
For non-trivial reviews, apply `reasoning-discipline` first. When weighing finding severity or choosing a verdict, use its Evaluate move: derive and weight the criteria that matter, and let a critical failure outrank a high score on a minor one.
